...
export async function POST(request: Request) {
  ...
  if (token && deviceId) {
    return handleDesktopEvaluation(request, token, deviceId)
  }

  return handleWebEvaluation(request)
}

async function handleDesktopEvaluation(request: Request, token: string, deviceId: string) {
  try {
...
async function handleWebEvaluation(request: Request) {
