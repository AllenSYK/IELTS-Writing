...
test('practice page exposes a centered draft manager and complete-test configuration', async () => {
  ...
  assert.match(selector, /<a[\s\S]*?href=\{buildHref\(mode\.mode, selection\)\}/)
  assert.match(selector, /window\.location\.assign\(buildHref\(mode, selection\)\)/)
  assert.doesNotMatch(selector, /createManagedDraft/)
  assert.doesNotMatch(selector, /refreshUser/)
  assert.doesNotMatch(selector, /startingRef/)
  assert.match(selector, /className="mode-card-header"/)
  assert.doesNotMatch(selector, /<button[\s\S]*?<GlassPanel className=\{`mode-card/)
  assert.match(editor, /createManagedDraft\(mode, requestedSelection\)/)
