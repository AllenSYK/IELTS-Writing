-- ============================================================
-- Privacy consent tracking - deploy SQL
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'privacy_consents_type_check' AND conrelid = 'public.privacy_consents'::regclass) THEN
    ALTER TABLE public.privacy_consents ADD CONSTRAINT privacy_consents_type_check CHECK (consent_type IN ('privacy_policy', 'terms', 'cross_border_transfer', 'minor_guardian'));
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'privacy_consents_status_check' AND conrelid = 'public.privacy_consents'::regclass) THEN
    ALTER TABLE public.privacy_consents ADD CONSTRAINT privacy_consents_status_check CHECK (consent_status IN ('granted', 'withdrawn'));
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_privacy_consents_user_type ON public.privacy_consents(user_id, consent_type, consent_status);

CREATE UNIQUE INDEX IF NOT EXISTS idx_privacy_consents_active ON public.privacy_consents(user_id, consent_type) WHERE consent_status = 'granted';
