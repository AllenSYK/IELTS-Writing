...
-- Index for finding active jobs per user
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'study_plan_gen_jobs_select_own' AND tablename = 'study_plan_generation_jobs'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'study_plan_gen_jobs_insert_own' AND tablename = 'study_plan_generation_jobs'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'study_plan_wallets_select_own' AND tablename = 'study_plan_adjustment_wallets'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'study_plan_adj_txn_select_own' AND tablename = 'study_plan_adjustment_transactions'
  ) THEN
...
-- RPC: Award adjustment points (server-side only, idempotent)
...
BEGIN
  ...
  v_amount := CASE
    ...
    WHEN p_task_type = 'error_review' THEN 1
    WHEN p_task_type IN ('grammar_drill', 'vocabulary_drill', 'review', 'timed_practice') THEN 1
    ELSE 1
  END;

  -- Check daily cap (skip for bonus)
  ...
  -- Cap the award if it would exceed daily limit
  ...
  IF NOT v_wallet_exists THEN
    INSERT INTO public.study_plan_adjustment_wallets (user_id, balance, lifetime_earned, lifetime_spent)
...
EXCEPTION
  WHEN unique_violation THEN
    -- Idempotency key already exists, return success without double-awarding
...
BEGIN
  ...
  INSERT INTO public.study_plan_adjustment_transactions (user_id, type, amount, reason, idempotency_key, balance_after)
  VALUES (p_user_id, 'spend', p_amount, p_reason, p_idempotency_key, v_new_balance);
...
BEGIN
  ...
  INSERT INTO public.study_plan_adjustment_transactions (user_id, type, amount, reason, idempotency_key, balance_after)
  VALUES (p_user_id, 'refund', p_amount, p_reason, p_idempotency_key, v_balance);
...
COMMENT ON TABLE public.study_plan_generation_jobs IS 'Background study plan generation jobs with progress tracking';
COMMENT ON TABLE public.study_plan_adjustment_wallets IS 'User wallets for study plan adjustment points';
COMMENT ON TABLE public.study_plan_adjustment_transactions IS 'Audit log for all adjustment point changes';
